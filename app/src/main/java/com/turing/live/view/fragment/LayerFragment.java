...
public class LayerFragment extends Fragment implements View.OnClickListener {
     ...
     * 界面相关
     */
    private TextView tvNickName,tvRoomName,tvSignature; //昵称 房间名 公告
    private Button btnFollow; //是否关注
    private ImageView ivClose; //关闭直播间
    ...
    private Timer timer;
    private View.OnClickListener followClickCallback = null; //关注点击事件

    public LayerFragment(View.OnClickListener followClickCallback){
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        ivHead = (CustomRoundView)view.findViewById(R.id.iv_head);
        tvNickName = (TextView)view.findViewById(R.id.tv_nickname);
        tvRoomName = (TextView)view.findViewById(R.id.tv_roomname);
        ...
        btnFollow.setOnClickListener(followClickCallback);
    ...
    public void onClick(View v) {
        switch (v.getId()){
            case R.id.iv_close:/*关闭*/
                ActivityManager.finishActivity(PLVideoTextureActivity.class.getSimpleName());
                break;
            case R.id.tvChat:/*聊天*/
                showChat();
                break;
     ...
     * 刷新房间头部信息
     */
    public void refreshHeadInfo(BaseRoomInfo baseRoomInfo){
        BaseRoomInfo.RoominfoBean roominfoBean = baseRoomInfo.getRoominfo();
        BaseRoomInfo.HostinfoBean hostinfoBean = baseRoomInfo.getHostinfo();

        Glide.with(getActivity()).load(hostinfoBean.getAvatar()).into(ivHead);
        tvNickName.setText(hostinfoBean.getNickName());
        tvRoomName.setText(roominfoBean.getName());

        if (TextUtils.isEmpty(hostinfoBean.getSignature())){
            tvSignature.setVisibility(View.GONE);
        }else{
            tvSignature.setVisibility(View.VISIBLE);
            tvSignature.setText(hostinfoBean.getSignature());
        }

        btnFollow.setText(baseRoomInfo.isFollow() ? "取关" : "关注");
    }
     ...
     * @param listData
     */
    public void setRoomUserListData(List<RoomUser> listData){
        audienceAdapter.setDatas(listData);
    }
